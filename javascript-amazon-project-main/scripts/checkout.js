...
function renderOrderSummary () {
  ...
  cart.forEach(cartItem =>{
    ...
    const matchingProduct = products.find(product => product.id === productId); //codice diverso dal corso ma piu efficace
    ...
    const deliveryDelay = deliveryOption.deliveryDays
    const today = dayjs();
    const deliveryDate = today.add(deliveryDelay, 'days');
      ...
      <div class="cart-item-details-grid">
        ...
        <div class="cart-item-details">
          ...
          <div class="product-quantity js-product-quantity">
            ...
            <span class ="save-quantity quantity-input link-primary js-save-quantity" data-product-id="${matchingProduct.id}">Save</span>
            <span class="delete-quantity-link js-delete-link link-primary" data-product-id="${matchingProduct.id}">
              Delete
            </span>
            <div class = "error-message shown js-error-message-${matchingProduct.id}" ></div>
          </div>
        </div>
  ...
  function updateCartQuantity () {
      ...
      function selectNewQuantity(link){
        const productId= link.dataset.productId;
          const container = document.querySelector(`.js-cart-item-container-${productId}`)
          ...
          if (inputValue >= 0 && typeof inputValue === 'number')  {
            ...
            document.querySelector(`.js-error-message-${productId}`).innerHTML = 'Inser a valid quantity!'
            setTimeout(() => {
              document.querySelector(`.js-error-message-${productId}`).classList.remove('shown')},3000);
          }
      }
